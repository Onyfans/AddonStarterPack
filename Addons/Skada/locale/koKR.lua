...
-- L["targets"] = ""
L["Highlight player"] = "하이라이트 플레이어"
L["If true, highlight the player's name in a different color"] = "활성화되면 플레이어의 이름을 다른 색상으로 강조 표시합니다."
L["Player highlight color"] = "플레이어 하이라이트 색상"
L["If 'Highlight player' is enabled, this determines the color of the player's name"] = "'하이라이트 플레이어'가 활성화되면 플레이어 이름의 색상이 결정됩니다."
L["Data update frequency"] = "데이터 업데이트 빈도"
L["How many times per second the data view will be updated"] = "초당 데이터 보기가 업데이트되는 횟수"
